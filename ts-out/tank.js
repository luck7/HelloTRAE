import { TANK_SIZE, BULLET_SIZE, GRID, CANVAS_W, CANVAS_H, DIR } from './constants.js';
...
export class Tank {
    ...
    move() {
        ...
        if (this.dir === DIR.UP)
            ...
            nx -= step;
        if (nx < 0 || ny < 0 || nx + this.width > CANVAS_W || ny + this.height > CANVAS_H)
            return;
        if (!checkMapCollision(nx, ny, this.width, this.height))
            return;
        if (!checkTankCollision(this, nx, ny))
            return;
        this.x = nx;
        this.y = ny;
    }
    shoot() {
        if (!this.alive || this.shootCooldown > 0)
            return;
        const activeBullets = game.bullets.filter(b => b.alive && b.owner === this).length;
        if (activeBullets >= 2)
            return;
    ...
    draw(ctx) {
        if (!this.alive)
            return;
        if (this.invincible > 0 && this.blinkTimer % 8 < 4)
            return;
